...
function initMobileMenu() {
    const container = document.getElementById('mobile-planet-btns');
    if (container.children.length > 0) return;
    
    const allCelestial = [{name: 'Sun', size: 20, dist: 0}, ...celestialData];
    allCelestial.forEach(d => {
        const btn = document.createElement('button');
        btn.className = 'btn-sci';
        btn.innerText = d.name.toUpperCase();
        btn.onclick = () => jumpTo(d.name);
    ...
    const speedRange = document.getElementById('mobile-speedRange');
    const speedVal = document.getElementById('mobile-speedVal');
    if (speedRange && speedVal) {
        speedRange.addEventListener('input', () => {
            const spd = parseFloat(speedRange.value);
            let text;
            if (spd >= 365) text = (spd / 365).toFixed(2) + 'y/s';
            else if (spd >= 1) text = spd.toFixed(1) + 'd/s';
            else text = (spd * 10).toFixed(1) + 'd/10s';
            speedVal.innerText = text;
            document.getElementById('speedRange').value = spd;
        });
    }
    
    const eventsList = document.getElementById('mobile-timeline-events');
    if (eventsList && eventsList.children.length === 0) {
        astronomicalEvents.slice(0, 10).forEach(event => {
            const div = document.createElement('div');
            div.textContent = `${event.year}: ${event.name}`;
            div.className = 'cursor-pointer hover:text-cyan-400 py-1';
            div.addEventListener('click', () => {
                currentDate.setFullYear(event.year);
                currentDate.setMonth(event.month - 1);
                currentDate.setDate(event.day);
